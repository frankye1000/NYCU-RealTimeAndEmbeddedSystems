...
INT8U getPrioHightRdy(){
    OS_TCB    *ptcb;
    INT8U prioHighRdy=OS_IDLE_PRIO;  //OS_IDLE_PRIO是最低priority
    INT16U deadLine=10000;
    ptcb = OSTCBList;                                  /* Point at first TCB in TCB list           */
    while(ptcb->OSTCBPrio==1 || ptcb->OSTCBPrio==2 || ptcb->OSTCBPrio==3 || ptcb->OSTCBPrio==0){  //3個task
        if(ptcb->OSTCBStat==OS_STAT_RDY && !ptcb->OSTCBDly && ptcb->deadLine<deadLine){           //沒有OSTCBDly而且目前小於目前priority才更新任務
...
void  OSStart (void)
{
    ...
    if (OSRunning == FALSE) {
        OSPrioHighRdy = 0;  //任務開始最高priority=0??
